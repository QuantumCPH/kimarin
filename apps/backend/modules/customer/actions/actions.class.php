...
//			echo 'validating form';
            if ($validated) {
                ...
                $this->getUser()->setFlash('message', $this->getContext()->getI18N()->__('%1% account is successfully charged with %2% %3%.', array("%1%" => $customer->getMobileNumber(), "%2%" =>$request->getParameter('charge_amount'), "%3%" => sfConfig::get('app_currency_code'))));
...
//                                echo '<br />';
    ...
    public function executeSelectRefillCustomer($request) {
        ...
        $ct->addOr(TransactionDescriptionPeer::TRANSACTION_TYPE_ID, 4); // For Bonous
